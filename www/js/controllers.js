angular.module('directory.controllers', [])
    ...
    .controller('ProductListCtrl', function ($scope, $location, $http, $cookies, Products, Authentication, $window) {
        Authentication.isAuth().then(function () {
            ...
            if ($location.$$host == 'localhost')
                ...
                $window.location.href = 'https://progweb.herokuapp.com/';
        });
    })
    .controller('ProductDetailCtrl', function ($scope, $stateParams, $http, $sce, Products, Authentication) {
        Authentication.isAuth().then(function () {
            $http.get('/dyn').then(function (response) {
                console.log('response.data.bby-' + response.data.bby);
                var bby = response.data.bby == 'Y';
    ...
    .controller('ProductBuyCtrl', function ($scope, $ionicModal, $stateParams, Products, Authentication) {
        Authentication.isAuth().then(function () {
            $scope.contacts = [
    ...
    .controller('OrderConfirmationCtrl', function ($scope, $location, $cookies, $http, $timeout, $filter, Authentication) {
        Authentication.isAuth().then(function () {
            $cookies.mySearch = "";
            ...
            if (true) {
                $http.get('/dyn?interval=GET').then(function (res) {
                    ...
                    $timeout(function () {
                        $scope.callAtTimeout(orderId, time);
                    }, time * 1000);
                });
            }
        }, function (err) {
